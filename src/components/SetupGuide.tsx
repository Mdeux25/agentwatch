...
import { checkClaudeInstalled } from '../lib/tauri'
...
const INSTALL_CMDS: Record<Platform, { label: string; cmd: string }[]> = {
  mac: [
...
export function SetupGuide({ onDone }: Props) {
  ...
  const cmds     = INSTALL_CMDS[platform]
  ...
  useEffect(() => {
    checkClaudeInstalled().then(v => {
      if (v) { setVersion(v); setCheckState('found'); setStep(3) }
      else   { setCheckState('not-found'); setStep(1) }
    })
  }, [])

  const recheck = async () => {
    setRechecking(true)
    const v = await checkClaudeInstalled()
    setRechecking(false)
    if (v) { setVersion(v); setCheckState('found'); setStep(3) }
    else    { setCheckState('not-found') }
  }
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 32, textAlign: 'center' }}>
          ...
          <p style={{ margin: '8px 0 0', fontSize: 14, color: 'rgba(255,255,255,0.45)' }}>
            AgentWatch needs Claude Code installed on your machine.
          </p>
        </div>
        ...
        {step === 0 && (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>Checking for Claude Code…</p>
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ margin: '0 0 20px', fontSize: 13, color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>
              ...
              The native installer is recommended — it auto-updates in the background.
            </p>
            ...
            <p style={{ margin: '16px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.3)', lineHeight: 1.5 }}>
              Requires macOS 13+, Windows 10+, or Ubuntu 20+. After install, restart your terminal.
            </p>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{
              ...
              A free account won't have Claude Code access.
            </div>
        ...
        {step === 3 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 700, color: '#f1f1f5' }}>
            ...
            <p style={{ margin: '0 0 28px', fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>
              Open a project folder in AgentWatch, then start a Claude Code session
              in your terminal inside that folder. Activity will appear automatically.
            </p>
            <button
              onClick={onDone}
              style={{
